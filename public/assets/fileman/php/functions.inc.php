...
function getFilesPath(){
  ...
  if(!$ret){
    $ret = RoxyFile::FixPath(BASE_PATH.'/Uploads/'.getUserDir());
...
//        $ret = "$base/Uploads/common";
//    }
//    else {
//        $ret = "$base/Uploads/".getUserDir();
//    }
    return $ret;
}
...
function getUserDir() {
	if(!isset($_SESSION["userID"]) || intval($_SESSION["userID"]) == 0) {
		error_log("fileman error, user_id not set");
		return "unknown_user";
	}
	//error_log("sess user id : " .  $_SESSION["userID"]);
	$userId = intval($_SESSION["userID"]);
	if($userId > 318) {
		return "usern-" . base_convert($userId, 10, 16); 
	} else {
		return "user-".substr(md5($_SESSION["userID"]), 0, 5);
	}
}

function getURL($path) {
    $url_base = "http://".$_SERVER["HTTP_HOST"];
    $url_base .= substr($_SERVER["REQUEST_URI"], 0, strrpos($_SERVER["REQUEST_URI"], "/"));
    $url_base .= "Uploads/".getUserDir();

    if(is_file($path)) {
        $url_base .= "./".basename($path);
    }
