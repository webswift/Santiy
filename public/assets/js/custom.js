...
function showFormError(message, id) {
	if (id == undefined)
		id = "error";

	$("#" + id).html('<label class="error">' + message + '</label>').css("display", "block");
}

function showFieldError(message, fieldID) {
	jQuery("#" + fieldID).closest('.form-group').removeClass('has-success').addClass('has-error').find("label.error").remove();
	jQuery("#" + fieldID).after('<label for="' + fieldID + '" class="error">' + message + '</label>');
}

function hideErrors() {
	jQuery(".form-group").removeClass("has-error").find("label.error").remove();
	jQuery(".form-group").removeClass("has-error").find("span.help-block").html('');
