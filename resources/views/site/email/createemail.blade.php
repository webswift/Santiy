...
#fileDetails .remove-file{
	...
	<div class="pageheader"><h2><i class="fa fa-envelope-o"></i> Create a new email template</h2></div>
								...
								<div class="form-group">
                                      ...
                                      @if($user->userType == "Team")
                                          <div class="ckbox ckbox-primary mt10 col-sm-4">
                                            <input type="checkbox" id="shareWithTeam" checked/>
				...
				<div class="panel-footer">
					<div class="col-sm-12">
						<p class="pull-right">
							...
							<button onclick="saveEmailTemplate()" class="btn btn-success">Save</button>
						</p>
					</div>
				</div>
			</div><!-- panel-body -->
...
function saveEmailTemplate() {
	...
	if(templateName == '') {
		...
		$.ajax({
			method: 'POST',
			url: "{{URL::route('user.email.addnewtemplate')}}",
			dataType: "JSON",
			data: formData,
			contentType : false,
			processData : false,
			success: function (obj) {
				...
				if(obj.success == "success") {
					...
					showError('There is some error in uploading attachments. Please try again.')
