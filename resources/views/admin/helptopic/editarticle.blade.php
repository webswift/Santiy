        ...
        <div class="col-sm-9 col-lg-10">
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group col-sm-12">
                            <input type="text" id="articleName" placeholder="Article Name" class="form-control" value="{{ $articleDetails->articleName }}"/>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-12">
                        <div class="col-md-5">
                            <div class="form-group">
                                <select id="topicID" class="form-control mb15">
                                    ...
                                    @foreach($topicLists as $topicList)
                                    <option value="{{ $topicList->id }}" @if($topicList->id == $articleDetails->topicID) selected @endif >{{ $topicList->topic }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="form-group form-horizontal">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" id="keywords" class="form-control" value="{{ $articleDetails->keywords }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-12">
                        <textarea id="articleText" placeholder="Your message here..." class="form-control" rows="20">{{ $articleDetails->text }}</textarea>
                    </div>
                ...
                <div class="panel-footer">
                    <button onclick="editArticle({{ $articleDetails->id }})" class="btn btn-primary">Save</button>
                </div>
            </div><!-- panel -->
...
<script>
    ...
    function editArticle(articleID)
    {
        ...
        }else if(articleText == '')
        {
            ...
            $.ajax({
                type: 'post',
                url: "{{ URL::route('admin.helptopics.editarticle') }}",
                cache: false,
                data: {"articleID": articleID, "articleName": articleName, "topicID": topicID, "keywords": keywords, "articleText": articleText},
                success: function(response) {
                    var obj = jQuery.parseJSON(response);
