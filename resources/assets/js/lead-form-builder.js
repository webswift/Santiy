				...
				if(obj.success == "success") {
					...
					ajaxFieldArray.forEach(function(fields) {
						...
						if(currentFieldName == 'Company Name') {
							...
							fieldID = "address";
							$('.sortable').append('<li class="'+fieldID+'"><span class="selectedFields" rel="'+type+'">Post/Zip code</span>' + 
									'<span class="pull-right fieldSmallBtn" style="margin-right: 39px"><input title="Is field required?" type="checkbox" '+required+'> </span></li>');
						}
							...
							if(type == 'text' || type == 'date'){
								removeFieldHtml += '<span title="Rename field" onclick="renameThisField(this)" class="pull-right fieldSmallBtn" style="margin-right: 8px">' + 
									'<i class="fa fa-pencil"></i></span>';
							} else {
								removeFieldHtml += '<span title="Edit field" onclick="editDropDownField(this)" class="pull-right fieldSmallBtn" style="margin-right: 8px">' + 
									'<i class="fa fa-pencil"></i></span>';
							}
							...
							if(type == 'text'){
								...
								$('.sortable').append(
										'<li>' + 
										'<span class="selectedFields" rel="'+type+'" data-original-name="'+currentFieldName+'">'+currentFieldName+ '</span>' + 
										removeFieldHtml +
										requiredCheckBoxCustomHtml + '</li>'
										);

							} else if(type == 'date'){

								$('.sortable').append(
										'<li><i class="mr5 glyphicon glyphicon-calendar"></i>' + 
										'<span class="selectedFields" rel="'+type+'" data-original-name="'+currentFieldName+'">'+ currentFieldName+ '</span>' + 
										removeFieldHtml +
										requiredCheckBoxCustomHtml + '</li>'
										);
								...
								$('.sortable').append(
										'<li><span class="selectedFields" rel="'+type+'" data-original-name="'+currentFieldName+'" data-name="'+currentFieldName+
										'" data-original-value="'+fields.values+'" data-value="'+fields.values+'">'+currentFieldName+' - '+fields.values+'</span>' + 
										removeFieldHtml + 
										requiredCheckBoxCustomHtml + '</li>'
										);
...
$(".predefindedFields").click(function() {
	...
	if($(this).is(':checked')) {
		if(value == 'Address') {
			$('.sortable').append('<li class="'+id+'"><span class="selectedFields" rel="text">Post/Zip code</span>' + 
					'<span class="pull-right fieldSmallBtn" style="margin-right: 38px;"><input title="Is field required?" type="checkbox"></span> </li>'
					);
		}
		$('.sortable').append('<li class="'+id+'"><span class="selectedFields" rel="text">'+value+'</span>' + 
				btnHtmlRemoveField +
				'<span class="pull-right fieldSmallBtn" style="margin-right: 28px;"><input title="Is field required?" type="checkbox"></span></li>'
				);
		$('.sortable').sortable();
	}
	else {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if(obj.status == "success") {
						...
						var html = 
							'The Form "' + obj.form_name + '" is currently associated with the following active campaigns:<br /><br />'
							+ campaign_names
							+ '<br />In order to delete this form please mark these campaigns as finished first.';
						bootbox.dialog({
							title : 'Please Note!' 
							, message : html
							, buttons: {
...
function showDemoForm() {
	...
	window.open(
	  formShowDemoUrl + "?fieldArray="+ encodeURIComponent(newFormFields)+"&emailFieldExists="+emailFieldExists,
	...
	if(fieldType == 'forField' || fieldType == 'forDate') {
		...
		if(fieldValue === '') {
			...
			if(fieldType == 'forField') {
				$('.sortable').append('<li><span class="selectedFields" rel="'+field_type+'" data-original-name="'+fieldValue+'">'+ fieldValue+
						'</span> ' + removeFieldHtml  + 
						requiredCheckBoxHtml + '</li>'
						);
			} else {
				$('.sortable').append('<li><i class="mr5 glyphicon glyphicon-calendar"></i><span class="selectedFields" rel="'+field_type+'" data-original-name="'+fieldValue+'">'+ fieldValue+
						'</span> ' + removeFieldHtml  +
						requiredCheckBoxHtml + '</li>'
						);
			}
		...
		if(!error) {
			...
			$('.sortable').append('<li><span class="selectedFields" rel="dropdown" data-name="'+fieldValue+'" data-original-name="'+fieldValue+
					'" data-value="'+options.join(',')+
					'" data-original-value="" ' + 
					`data-options-changes='${JSON.stringify(optionsChanges)}'`+
					'>'+fieldValue+' - '+options.join(', ')+'</span>' + 
					removeFieldHtml  + 
					requiredCheckBoxHtml + '</li>'
					);
			$('.sortable').sortable();
